import { disableCompany } from '../../neo4jCalls/compnayCalls/disableCompany';
import { SUPER_ADMIN } from '../../constants/constants';
import { decodeToken, validateSession } from '../../utils/utils';
import { GenericReturn } from "../../entities/genericReturn";

const dsiableCompanyHandler = async (id: string, jwtToken: string | undefined): Promise<GenericReturn> => {

    console.log(`Initiating disableCompanyHandler \n`);
    ...
    try {
        ...
        const deletionResult = await disableCompany(id);
        ...
        result.message = `Error disabling company: ${error}`;
        return result;

    }
}

export default dsiableCompanyHandler;
